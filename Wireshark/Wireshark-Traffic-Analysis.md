...
With just this information alone, it is an abundance of evidence that points towards this being an IPv4 packet encapsualted in ICMP. The final two bytes are `03 4c`. We can treat this as a single number because the last part of the header is defined as one, 16-bit value. First we need to convert each to decimal. `03` is simply `3`, and `4c` can be treated as such:
- `4` = `4`
- `c` = `12`
...
`Google Chrome/83.0.4103.116 Windows` is a bit of an odd one structurally. _Typically_, you see the browser details at the end of the string (like in our other samples above). Also the OS details are usually listed in the comments (again, like the other samples). There was a version `83.0.4103.116` for Chrome, so that's not an indicator. We can mark this as the possible sixth offender and continue the analysis.
...
`Mozilla/5.0 (Windows; U; Windows NT 6.4; en-US) AppleWebKit/534.10 (KHTML, like Gecko) Chrome/8.0.552.237 Safari/534.10` is another interesting one. Structurally, it is sound. It starts with the Mozilla compatibility token, followed by the OS comments, the rendering engine and comments, and the browser being used. However, the versions of the items in this list don't make sense naturally. I couldn't find a _super_ reliable source for the Windows NT 6.4 release date, or an official publication of it's release at all. According to [this Reddit post](https://www.reddit.com/r/windows/comments/yzcwiw/nt_64windows_82/?rdt=53014), NT 6.4 was a pre-release version for Windows 10 preview. According to the [Windows NT Wikipedia](https://en.wikipedia.org/wiki/Windows_NT) page, the version jumps from 6.3 to 10 (10 being Windows 10 which we all know well). This somewhat confirms that Windows NT 10 was Windows NT 6.4 before being released to the public. To dispel any confusion, There is no difference between **Windows NT 10** and **Windows 10**. Windows 10 is the consumer product built on top of the Windows NT OS architecture. They are used interchangeably. With that being said, this would put "Windows NT 6.4" somewhere between October 17, 2013 and July 29, 2015. Probably sometime in 2014.
